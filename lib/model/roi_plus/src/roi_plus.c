...
void ROIPlusForwardCpu(const float* bottom_data, const float spatial_scale, const int num_rois,
                     const int height, const int width, const int channels,
                     const int aligned_height, const int aligned_width, const float * bottom_rois,
                     float* top_data);

void ROIPlusBackwardCpu(const float* top_diff, const float spatial_scale, const int num_rois,
                     const int height, const int width, const int channels,
                     const int aligned_height, const int aligned_width, const float * bottom_rois,
                     float* top_data);
                     ...
                     THFloatTensor * features, THFloatTensor * rois, THFloatTensor * output)
{
    ...
    // do ROIAlignForward
    ROIPlusForwardCpu(data_flat, spatial_scale, num_rois, data_height, data_width, num_channels,
...
int roi_plus_backward(int aligned_height, int aligned_width, float spatial_scale,
                       ...
                       THFloatTensor * bottom_grad, THFloatTensor * offset_x_grad,
                       THFloatTensor * offset_y_grad, THFloatTensor * feature_data)
{
    //Grab the input tensor
    ...
    float * offset_x_grad_flat = THFloatTensor_data(offset_x_grad);
    float * offset_y_grad_flat = THFloatTensor_data(offset_y_grad);

    float * feature_data_flat = THFloatTensor_data(feature_data);
    ...
    // do ROIAlignBackward
    ROIPlusBackwardCpu(top_grad_flat, spatial_scale, num_rois, data_height,
            data_width, num_channels, aligned_height, aligned_width, rois_flat, 
            bottom_grad_flat, offset_x_grad_flat, offset_y_grad_flat, feature_data_flat);
...
void ROIPlusBackwardCpu(const float* top_diff, const float spatial_scale, const int num_rois,
                     const int height, const int width, const int channels,
                     const int aligned_height, const int aligned_width, const float * bottom_rois,
                     float* bottom_diff, float* offset_x_diff, float* offset_y_diff, float* feature_data)
{
    const int output_size = num_rois * aligned_height * aligned_width * channels;
    ...
    for (int idx = 0; idx < output_size; ++idx)
    {
        ...
        if (h < 0 || h >= height || w < 0 || w >= width)
        {
            ...
            offset_x_diff += top_diff[idx] * ( - feature_data[upleft] * (1. - h_ratio) +
                                                     feature_data[upright] * (1. - h_ratio) +
                                                   - feature_data[downleft] * h_ratio + 
                                                     feature_data[downright] * h_ratio );
            
            offset_y_diff += top_diff[idx] * ( - feature_data[upleft] * (1. - w_ratio) +
                                                   - feature_data[upright] * w_ratio +
                                                     feature_data[downleft] * (1. - w_ratio) + 
                                                     feature_data[downright] * w_ratio );
