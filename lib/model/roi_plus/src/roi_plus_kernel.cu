...
#define CUDA_1D_KERNEL_LOOP(i, n)                            \
    ...
    __global__ void ROIPlusForward(const int nthreads, const float* bottom_data, const float spatial_scale, const int height, const int width,
    ...
    int ROIPlusForwardLaucher(const float* bottom_data, const float spatial_scale, const int num_rois, const int height, const int width,
        ...
        ROIPlusForward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
    ...
    __global__ void ROIPlusBackward(const int nthreads, const float* top_diff, const float spatial_scale, const int height, const int width,
                                     const int channels, const int aligned_height, const int aligned_width, float* bottom_diff, const float* bottom_rois, float* offset_x_diff, float* offset_y_diff, const float* feature) {
        CUDA_1D_KERNEL_LOOP(index, nthreads) {
            ...
            if (!(h < 0 || h >= height || w < 0 || w >= width)) {
                ...
                // calculate offset grad for x and y
                atomicAdd(offset_x_diff + index, top_diff[index] * ( - feature[upleft] * (1. - h_ratio) +
                                                                     feature[upright] * (1. - h_ratio) +
                                                                   - feature[downleft] * h_ratio + 
                                                                     feature[downright] * h_ratio ));
                                                                     
                atomicAdd(offset_y_diff + index, top_diff[index] * ( - feature[upleft] * (1. - w_ratio) +
                                                                   - feature[upright] * w_ratio +
                                                                     feature[downleft] * (1. - w_ratio) + 
                                                                     feature[downright] * w_ratio ));

            }
        }
    }

    int ROIPlusBackwardLaucher(const float* top_diff, const float spatial_scale, 
                                ...
                                float* offset_x_diff, float* offset_y_diff,
                                const float * feature, cudaStream_t stream) 
    {
        ...
        ROIPlusBackward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
          output_size, top_diff, spatial_scale, height, width, channels,
          aligned_height, aligned_width, bottom_diff, bottom_rois, offset_x_diff, offset_y_diff, feature);
